...
(function ()
{
  ...
  function setup() {
    ...
    // We can't rely on the background worker receiving an 'onconnect' callback in a timely fashion.
    // Sometimes it works (when opening a new tab, and doing a google search) whereas other times
    // it doesn't (starting opera with a search result tab saved from the last session).
    // Here we break the nice handshake abstraction and ID ourselves and request options manually.
    window.addEventListener('DOMContentLoaded', function() {
      background.postMessage({what: 'id', name: 'userjs'});
      background.postMessage({what: 'get userjs options'});
    }, false);
  }
  
  function start_monitoring_search_results() {
  ...
  // De-obfuscates googles redirect urls into direct urls and optionally
  // enables removing javascript based redirect.
  function process_result_link(a_tag) {
    var real_url = a_tag.href;
    ...
    if(google_hosts.some(function(host) { return host == a_tag.hostname})) {
      var matches = /[&?](url|q)=([^&]+)/.exec(a_tag.href);
      
      // Make sure we're in the right place
      if(matches && matches.length == 3 && /^\/url$/.test(a_tag.pathname)) {
        real_url = decodeURIComponent(matches[2]);
        a_tag.setUserData('original_url', original_url, null);
        a_tag.href = real_url;
      }
    }
    
    // It would be nice to do this live as with everything else,
    // but it's a bit hard to test as google tends to switch their
    // redirects on and off pseudorandomly.
    if(options['remove_redirects']) {
      a_tag.removeAttribute('onmousedown');
    }
  }
